"""GetXAPI X search source for the v3.0.0 last30days pipeline.

Uses the GetXAPI REST API (https://api.getxapi.com) to search X/Twitter
with full engagement metrics (likes, retweets, replies, quotes, views,
bookmarks). Requires an API key from getxapi.com.
"""
...
def _log(msg: str):
    log.source_log("GetXAPI", msg, tty_only=False)
...
def search_getxapi(
    ...
    Args:
        ...
        token: GetXAPI API key

    Returns:
        Dict with "items" list and optional "error" string.
    """
    if not token:
        return {"items": [], "error": "No GETXAPI_API_KEY configured"}
    ...
    for query_text in queries:
        try:
            url = f"{_BASE_URL}/twitter/tweet/advanced_search"
            q = f"{query_text} since:{from_date} until:{to_date}"
            full_url = f"{url}?q={_url_encode(q)}"
            ...
            if status in (401, 403):
                return {"items": [], "error": f"GetXAPI auth failed ({status})"}
...
def search_and_enrich(
    ...
    """Search X via GetXAPI and return results.

    GetXAPI returns full engagement data by default, so no separate
    enrichment step is needed.
    """
    return search_getxapi(topic, from_date, to_date, depth=depth, token=token)
