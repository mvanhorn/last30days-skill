...
- `--plan <json|path|->` - supply a host-authored query plan and skip the internal planner. Pass inline JSON, a JSON file path, or `-` to read JSON from stdin. Agent hosts should prefer `--plan -` with a quoted heredoc so apostrophes stay shell-safe and no temporary cleanup command is needed.
- `--competitors-plan <json|path|->` - supply per-entity comparison targeting as inline JSON, a JSON file path, or stdin with `-`. Implies competitor mode; see the comparison contract in `SKILL.md`.
...
- `--judgments <path|->` - leg 2: resume from the nominations bundle, applying host judgments (`{"bundle_id": "...", "judgments": [{"id", "name", "junk", "worthiness"}, ...]}`, bound to the bundle by `bundle_id`) from a file or stdin with `-`. Runs the per-topic research passes (deep tier by default; budget tunable via `LAST30DAYS_ENRICH_BUDGET_SECONDS` below), writes the pending report (`discover-pending.json`), and prints per-topic angle inputs. Requires `--discover`.
...
- `--angles <path|->` - optional host angles input for `--discover --finalize` (`{"bundle_id": "...", "angles": [{"id", "podcast", "x_article"}, ...]}`, sentences capped at 200 chars), read from a file or stdin with `-`; omitting it ships the brief without angle lines. All three protocol legs must share one `--save-dir` (handoff files live there, else in `~/.config/last30days/`); contract failures (missing/stale/unbound handoff files) exit 2 with the remedy on stderr, and `--mock` protocol legs require `--save-dir` to stay side-effect-free.
...
Pass as `--competitors-plan client/competitors-plan.json`, as an inline string, or as `--competitors-plan -` with a quoted heredoc. See `SKILL.md` section "If QUERY_TYPE = COMPARISON" for the full schema.
