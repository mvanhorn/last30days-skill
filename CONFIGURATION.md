...
| YouTube | `yt-dlp` CLI installed; `SCRAPECREATORS_API_KEY` adds a server-side transcript fallback used only when yt-dlp fails (429 / bot-gate); `YOUTUBE_PROXY` routes yt-dlp through a proxy (e.g. `socks5://127.0.0.1:1080`) for regions where YouTube is blocked | always on if `yt-dlp` present; SC transcript fallback default-on when key set (no credit spent unless yt-dlp fails) | yes |
