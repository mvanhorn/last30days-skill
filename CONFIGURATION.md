...
**YouTube via SSH (`LAST30DAYS_YOUTUBE_SSH_HOST`).** When a datacenter-hosted run hits YouTube's bot-wall (ytsearch returning 0 results), route yt-dlp through a residential machine: set `LAST30DAYS_YOUTUBE_SSH_HOST=<ssh-alias>` (a plain alias from `~/.ssh/config`, e.g. `macmini`) and the remote host must have yt-dlp installed. Availability, diagnostics, and execution all use the same SSH-aware check: with the alias set (and a local `ssh` binary), YouTube is available via the `ssh -o BatchMode=yes` route; without it, YouTube is unavailable — never advertised as active. `--diagnose` reports the route (`local`, `ssh`, or `unavailable`) under `ytdlp_route`. On macOS remotes, add Homebrew's shellenv to `~/.zshenv` (not just `~/.zprofile`) so non-login SSH shells find yt-dlp on PATH. The value must match `[A-Za-z0-9._-]+`; anything else is ignored with a warning.
