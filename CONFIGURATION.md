...
| X / Twitter | one of (auto chain order): browser cookies for Bird (`AUTH_TOKEN`+`CT0`, `FROM_BROWSER`, the `agentcookie` sidecar, or a live Chrome CDP session), a signed-in `grok` CLI (fail-closed backup, no X credential), `XAI_API_KEY`, `XQUIK_API_KEY`, or `SCRAPECREATORS_API_KEY` | X items in results | cookie-jar / Bird = free; grok = no X credential, draws on your Grok plan; Xquik / xAI / ScrapeCreators = key-based |
...
**X backend priority (bird first).** The default X backend chain is bird (browser cookies) → grok (signed-in Grok CLI, fail-closed) → xai (API key) → xurl (OAuth2 CLI) → xquik (API key). Cookies beat everything: bird wins whenever a complete `auth_token`+`ct0` pair is available. grok sits second as a fail-closed backup (see below), so a leftover `~/.grok/auth.json` never steals first place over bird and never blocks the fall-through to xai when the grok session is dead or expired.

**X cookie discovery for bird.** bird needs an `auth_token`+`ct0` pair. Rather than only reading a browser cookie DB, the engine walks these sources in order and takes the FIRST source that yields a COMPLETE pair (no half-pair merge across sources), all under the browser-cookie read consent (`FROM_BROWSER`/setup `--allow-browser-cookies`; `--preflight`/`--diagnose` read nothing):

1. an explicit `AUTH_TOKEN`+`CT0` already in your environment/`.env` (never overwritten);
2. the [`agentcookie`](https://github.com/) sidecar CLI, if on PATH — `agentcookie cookies --domain .x.com --json`. This is a soft dependency (absent = skipped) and is **independent of `FROM_BROWSER`**; set `AGENTCOOKIE=off` to disable it. It delivers cookies on Linux, where the on-disk Chrome store can't be decrypted here;
3. a live Chrome/Chromium session over the DevTools Protocol (CDP): the engine scans reachable localhost debug ports (the usual box-Chrome port is `9222` plus the X display number, plus a small range) and calls `Network.getAllCookies`. Sign into x.com in Chrome and leave it running with a remote-debugging port open. `FROM_BROWSER=off` skips CDP (and the browser extractor below);
4. the in-process browser cookie extractor (Firefox on Linux; Chrome/Firefox/Safari on macOS), honoring `FROM_BROWSER`.

Discovered cookies are held in memory for the run only — they are **never written** to `~/.config/last30days/.env`, and cookie values are never printed. `--diagnose`/`doctor` name the source that will serve X (`will use: bird` when a cookie source — a `FROM_BROWSER` browser or the `agentcookie` sidecar on PATH — is present).

**Grok CLI (fail-closed backup).** Install the Grok CLI (`curl -fsSL https://x.ai/cli/install.sh | bash`) and run `grok login`, and X can work with no X account, no browser cookies, and no `XAI_API_KEY`. grok is a **fail-closed backup after bird**: it is auto-selected only when the CLI is signed in with an affirmatively-valid session, so a leftover or expired `~/.grok/auth.json` never steals first place over bird and never blocks the fall-through to xai. You can still force it exclusively with `LAST30DAYS_X_BACKEND=grok` (a pin also accepts an expired session so the run-time refresh gets a chance). It is not "free" in the way the cookie path is: calls draw on your Grok plan, and depth costs several calls per run because the underlying tool caps each search at 10 posts. Results are validated before use — every returned post's ID is decoded to confirm it falls inside the requested date range, because the retrieval is performed by a language model and can otherwise return confident, well-formed posts that were never searched for.
...
| `LAST30DAYS_X_BACKEND` | Pins the X backend (`bird` / `grok` / `xai` / `xurl` / `xquik`); doctor renders the pin and predicts "will use" accordingly. The unpinned auto chain is bird → grok → xai → xurl → xquik, where grok is fail-closed (auto-selected only when signed in with a valid session, so a dead/expired `~/.grok/auth.json` never blocks the fall-through to xai). A pin forces one backend with no failover. |
| `AGENTCOOKIE` | Set to `off` to disable the `agentcookie` sidecar as an X cookie source for bird. When unset and `agentcookie` is on PATH, the engine reads `auth_token`+`ct0` from it (independent of `FROM_BROWSER`) as the second cookie source after an explicit env pair. Never writes cookies to disk. |
